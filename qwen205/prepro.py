...
def preprocess_text(text, max_context):
    """
    For Generative Model such as Qwen and Llama, since the max context length for most langauge models are 32k, 
    we will have max_context of 7k each, for 1-shot, we have to provide 3 examples + 1 evaluation text = 4x7k = 28k
    Also to make it more manageable due to limited computational resources. 
    For generalizablility, we use sent_tokenizer and word_tokenizer.
    Tokenization and the text generation will go through the pipeline approach.
    """
