...
class DocClassificationDataset(Dataset):
    def __init__(self, docs, attn_masks, tfidf_weights, labels):
        self.docs = docs
    ...
    def __getitem__(self, idx):
        ...
        tfidf_weight = self.tfidf_weights[idx]
        label = self.labels[idx]
        return doc, attn_mask, tfidf_weight, label
        ...
        for batch in tqdm(dataloader, desc="Training"):
            ...
            with autocast():
                outputs, _, _, document_embs = self.model(
                    ...
                    batch['tfidf_weights'].to(self.args.device)
                )
...
def main():
    ...
    parser.add_argument("--risk_metric", choices=["std", "skew", "kurt", "sortino"], default="skew", type=str)
    ...
    wandb.init(
        name="tfidfxra" + f"_{args.test_year}" + f"_S{args.seed}",
    ...
    train_docs, train_attn_masks, train_tfidf_weights, train_labels = load_from_hdf5(f"../processed/{args.test_year}/tfidfhans/train_preprocessed.h5")
    test_docs, test_attn_masks, test_tfidf_weights, test_labels = load_from_hdf5(f"../processed/{args.test_year}/tfidfhans/test_preprocessed.h5")

    train_dataset = DocClassificationDataset(train_docs, train_attn_masks, train_tfidf_weights, train_labels[:, risk_metric_map[args.risk_metric]])
    test_dataset = DocClassificationDataset(test_docs, test_attn_masks, test_tfidf_weights, test_labels[:, risk_metric_map[args.risk_metric]])
