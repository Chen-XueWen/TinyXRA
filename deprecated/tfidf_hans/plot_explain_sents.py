...
for metric_name, metric_idx in metrics_info.items():
    ...
    for year in years:
        ...
        for seed in seeds:
            explain_result_loc = f"explain_experiments/sents/{risk_metrics}/Y{year}S{seed}.json"
    ...
    plt.xlabel('% of top attended sentences removed')
    plt.ylabel(f'{metric_name.capitalize()} Score')
    #plt.title(f'{metric_name.capitalize()} Score vs Removed Sentences ({risk_metrics})')
    plt.legend(loc='lower left')
    plt.grid(True)

    # Save each metric's figure
    plt.savefig(f'images/{risk_metrics}_{metric_name}_multi_year_sents_plot.png', dpi=300, bbox_inches='tight')